...
<div id="reviewattpage">
	<?php require('../teacher/teacherdata.php');?>
					...
					<select name="classvalue" id="classvalue" required>
						...
						<?php
							for($i=0;$i<sizeof($subids);$i++)
							{
								...
								echo(" <option value = '".$classvalue."'> ".ucwords($subnames[$i]).": ".ucwords($years[$i])." ".ucwords($coursenames[$i])." (Div ".ucwords($divisions[$i]).") </option> ");
							}
						?>
					</select>
				</h4>
